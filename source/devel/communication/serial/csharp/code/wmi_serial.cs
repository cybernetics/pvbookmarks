 ...
 * 
 * 
Attention: la classe win32_serialport ne détecte pas les ports USB série.
...
namespace VideoCell
{
    ...
    public class CWin32SerialPort
    {
        ...
        /// </summary>
        public static List<CWin32SerialPort> GetListWin32SerialPortVIDEOCELL(bool with_log)
        {
            List<CWin32SerialPort> win32SerialPortList = CWin32SerialPort.GetListWin32SerialPort();
            if (win32SerialPortList == null)
            {
                Log.Write("No serial port detected", with_log);
                return null;
            }
            else
            {
                Log.Write("Serial port detected", with_log);
            }
        ...
        /// Retourne la liste des ports série win32 VIDEOCELL
        /// </summary>
        public static CWin32SerialPort GetVideocellSerialPort(bool with_log)
        {
            List<CWin32SerialPort> win32SerialPortVideoCellList = GetListWin32SerialPortVIDEOCELL(with_log);

            if (win32SerialPortVideoCellList == null)
            {
                // Aucune carte videocell n'a été détecté
                Log.Write("No videocell card detected", with_log);
                return null;
            }
            else
            {
                Log.Write("Videocell card detected", with_log);
            }
