...
::
    ...
    Martin Pool <mbp@sourcefrog.net> via lists.idyll.org 
    à:   Chris Wesseling <Chris.Wesseling@cwi.nl>
    cc:  Testing in Python <testing-in-python@lists.idyll.org>
    date:    17 juin 2013 23:58
    objet:   Re: [TIP] shell-like tests
    liste de diffusion:  testing-in-python.lists.idyll.org Filtrer les messages de cette liste de diffusion
    envoyé par:  lists.idyll.org



Thanks for mentioning those two. On a brief read the differences seem 
to be:

- Cram runs test scripts located in separate files, and can patch them 
  with the differences, which would make it great for testing documentation 
  examples, and maybe a bit harder if you want to run some Python setup 
  or mocking code around the code under test.
